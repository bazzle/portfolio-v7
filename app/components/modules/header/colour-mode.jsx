...
function ColourMode() {
	const [colourModeState, setColourModeState] = useState('dark');

	const applyLightMode = () => {
		document.body.classList.add('light-mode');
		localStorage.setItem('colourMode', 'light');
		setColourModeState('light');
	}
	const applyDarkMode = () => {
		document.body.classList.remove('light-mode');
		localStorage.setItem('colourMode', 'dark');
		setColourModeState('dark');
	}
	...
	return (
		<button onClick={handleClick} aria-label={`Switch to ${colourModeState === 'light' ? 'dark' : 'light'} mode`} className={styles.dm_switcher}>
